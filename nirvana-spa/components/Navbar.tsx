...
import { NAV_LINKS, LOGO_URL, SPA_NAME } from '../constants';
...
const Navbar: React.FC<NavbarProps> = ({ onNavigate }) => {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-300 ease-in-out ${isScrolled ? 'bg-brand-cream shadow-lg py-2' : 'bg-transparent py-4'}`}>
        ...
        <div className="flex items-center justify-between h-16">
          <div className="flex-shrink-0">
            <a
              ...
              aria-label={`${SPA_NAME} Home`}
            >
              <img src={LOGO_URL} alt={`${SPA_NAME} Logo`} className="h-12 md:h-14 w-auto transition-all duration-300 ease-in-out" />
            </a>
          </div>
